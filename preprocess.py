...
def csv_preprocces(filepaths, test_ratio=0.2, valid_ratio=0.1, num_features=2100, 
    ...
    print('---------------------------------------------------------')
    rows_skipped = tf.data.experimental.cardinality(dataset_filepaths).numpy()
    instances = 1000000 - rows_skipped # 1M events generated and some rows skipped when read files.
